...
description: "Automate campaigns with triggers and actions—including calls, SMS, webhooks, and opportunities"
---

## Overview

Workflows let you automate follow-ups and routing across voice, chat, and external systems. Triggers can fire on call outcomes, Meta lead forms, inbound webhooks, and more—then actions run automatically (calls, SMS, webhooks, chat handoffs, and beyond).
...
1. **Trigger:** An event that starts the workflow (e.g., AI call completed, Meta form submitted, inbound webhook)
...
### Meta Ad Form Submitted

Trigger a workflow the moment a lead submits a **Meta ad lead form** (Facebook or Instagram). Ideal for speed-to-lead campaigns where every minute counts.
...
Start a workflow when an external system sends an HTTP request to your workflow’s inbound webhook URL. Use this to connect custom apps, landing pages, other SaaS tools, or middleware that can POST JSON payloads into Outbox AI.

**Common uses:**

- Fire automations from your own backend or a serverless function
- Chain tools like Zapier, Make, or n8n into Outbox workflows
- Kick off contact-specific sequences from product or billing events

<Note>
  Configure the webhook URL and authentication in the workflow trigger settings.
  Payload fields you send can often be mapped into contact data and workflow
...
Send an HTTP request to any URL you control—ideal for notifying external systems, Zapier/Make, or your own API when a workflow step runs.

**Configuration:**

- **URL** – Endpoint to call (often POST)
- **Method / headers** – Match what your receiver expects
- **Body** – JSON or text; use contact and workflow variables where supported

**Use cases:**

- Push lead or call outcomes into a data warehouse
- Trigger a process in another product the moment a workflow runs
- Mirror Outbox events into your internal tools

---

### Assign Chat Agent

Hand the contact’s conversation to a specific **chat agent** as part of the workflow. Use this when a call, form, or webhook should change who handles ongoing SMS, DMs, web chat, or email threads.

**Use cases:**

- Route Meta leads to a dedicated sales chat agent
- Escalate from a lightweight bot to a specialist agent after qualification
- Align chat ownership with tags or workflow branches

---

### Interrupt Chat Agent

Stop the active chat agent for a contact so automation pauses—useful when a human needs to take over or when you want to halt AI replies until something else happens.

**Use cases:**

- Pause AI after a high-value event until your team responds
- Prevent duplicate automated messages when a workflow also assigns manual follow-up
- Coordinate with **Interrupt Mode** and human handoff in the CRM

---

### Create Opportunity

Creates a new **opportunity** in a pipeline for the contact, with the stage and fields you configure in the action.

**Use cases:**

- Log a deal when a Meta lead form triggers a workflow
- Open an opportunity after a high-score AI call or webhook event
- Start a sales track before a rep assigns ownership manually

See [Opportunities](/opportunities) for pipelines and stages.

---

### Update Opportunity

Updates an existing opportunity—typically **moving a stage** or refreshing fields after an automated step.

**Use cases:**

- Advance a deal after a successful follow-up SMS or email step
- Sync pipeline status when a branch condition (score, tag, etc.) fires
- Close or disqualify opportunities from automated sequences
...
<Card title="Opportunities" icon="bullseye" href="/opportunities">
  Pipelines, stages, and opportunity tools
</Card>
