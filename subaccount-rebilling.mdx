...
With Stripe Wallet, your client connects their own Stripe account and you can create a dedicated wallet for each subaccount/client. Usage charges are then automatically deducted from that wallet as their AI agents run.
...
<Accordion title="Stripe Wallet (Recommended)">
  Connect your client's Stripe account and set up a dedicated wallet for that client. Usage is automatically charged from the wallet as the client uses AI chat and voice agents.
  **Best for:** Agencies that want automatic, usage-based charging with client-owned Stripe billing.
</Accordion>
...
<Accordion title="Start with Stripe Wallet for most clients">
  ...
  billing for both chat and voice. If you need a manual approval flow first, use
  Stripe Draft during onboarding and move to Stripe Wallet later.
</Accordion>
