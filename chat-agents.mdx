...
Use the **Follow Up Prompt** field to tell the chat agent how and when to **follow up with the contact later** if the conversation goes quiet—without you writing each message by hand.
...
The agent uses this instruction together with your main prompt, so keep it specific about timing, tone, and what to say (or not say) on each nudge.
...
## Message score and summary

Every chat message is analyzed for **quality and outcome**:
