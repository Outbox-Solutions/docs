...
Every save creates a prompt history entry so you can track changes and recover
earlier versions when needed.

**What it gives you:**

- A revision trail of prompt updates over time
- Safer experimentation with new instructions
- Fast rollback when a new version underperforms

**Suggested workflow:**

1. Make one focused change (tone, objection handling, tool rules, etc.)
2. Save and test with realistic scenarios
3. Compare outcomes
4. Restore a previous version from Prompt History if needed

<Tip>
  Use Prompt History together with scores/summaries in logs to identify which
  prompt revisions actually improved outcomes.
</Tip>
...
<Note>
  **Voice calls include automatic context:** Outbox AI injects summaries of the
  last **3 answered** calls, the current time, and the caller's phone number.
  You don't need to add these as variables. **Chat agents** also receive the
  current time in context—no separate time tool is required.
</Note>
...
<Steps>
  ...
  <Step title="Preview with Built-in Viewer">
  ...
  <Step title="Refine the Structure">
    Tweak headings, lists, and spacing until the preview looks crystal-clear. You can also use external tools like [Markdown Live Preview](https://markdownlivepreview.com/) if you prefer
  </Step>
